...
public class SectionRenderCache implements IBlockAccess {
	...
	private static final ArrayCache<BiomeGenBase> BIOME = new ArrayCache<>(48 * 48, BiomeGenBase[]::new, null);
	...
	protected Cache2D<BiomeGenBase> biomeCache;
	...
	protected final Function<BlockPos, BiomeGenBase> calculateBiomeRef = this::calculateBiome;
	...
	public SectionRenderCache(World world, SectionPos sectionPos) {
		...
		for (int x = minChunkX; x <= maxChunkX; x++) {
			for (int z = minChunkZ; z <= maxChunkZ; z++) {
				this.chunkCache.computeIfAbsent(x, z, world::getChunkFromChunkCoords);
	...
	public void initCaches() {
		...
		this.biomeCache = new Cache2D<>(minX - 16, minZ - 16, maxX + 16, maxZ + 16, BiomeGenBase.plains, BIOME.get());
	}
	...
	private int calculateCombinedLight(BlockPos pos, int minBlockLight) {
		...
		if (state.getBlock().getUseNeighborBrightness()) {
			...
			light = this.getLight(mutable.set(pos.getX(), pos.getY() - 1, pos.getZ()), light);
			light = this.getLight(mutable.set(pos.getX(), pos.getY() + 1, pos.getZ()), light);
			light = this.getLight(mutable.set(pos.getX(), pos.getY(), pos.getZ() - 1), light);
			light = this.getLight(mutable.set(pos.getX(), pos.getY(), pos.getZ() + 1), light);
			light = this.getLight(mutable.set(pos.getX() - 1, pos.getY(), pos.getZ()), light);
			light = this.getLight(mutable.set(pos.getX() + 1, pos.getY(), pos.getZ()), light);
		} else {
	...
	private int getLight(BlockPos pos, int sky, int block) {
		...
		if (section != null) {
			if (!this.world.provider.getHasNoSky() && sky < 15) {
				sky = Math.max(sky, LightUtil.getSkyLight(section, pos));
			}
			if (block < 15) {
				block = Math.max(block, LightUtil.getBlockLight(section, pos));
			}
		} else if (!this.world.provider.getHasNoSky() && sky < EnumSkyBlock.SKY.defaultLightValue) {
			Chunk chunk = this.getChunk(pos);
	...
	public boolean isAirBlock(BlockPos pos) {
		//IBlockState state = this.getBlockState(pos);
		return this.getBlockState(pos).getBlock().isAir((IBlockAccess)this, pos);// return state.getBlock().isAir(state, this, pos);
	}
	...
	private BiomeGenBase calculateBiome(BlockPos pos) {
		Chunk chunk = this.getChunk(pos);
		return chunk == null ? BiomeGenBase.plains : chunk.getBiome(pos, this.getWorld().getWorldChunkManager());//return chunk == null ? BiomeGenBase.plains : chunk.getBiome(pos, this.getWorld().getBiomeProvider());
	}

	@Override
	public int getStrongPower(BlockPos pos, EnumFacing direction) {
		IBlockState state = this.getBlockState(pos);
		return state.getBlock().getStrongPower((IBlockAccess)this, pos, state, direction);//return this.getBlockState(pos).getStrongPower(this, pos, direction);
	}
