...
public class RenderChunkTaskSortTranslucent extends AbstractRenderChunkTask<RenderChunk> {
	...
	public RenderChunkTaskResult run() {
		...
		VertexSortUtil.sortVertexData(vertexData, vboPart.getCount(), DefaultVertexFormats.BLOCK.getNextOffset(), 4,
		...
		taskDispatcher.runOnRenderThread(() -> {
			if (!this.canceled() && vboPart.isValid()) {
				...
				GL15.glBufferSubData(GL15.GL_ARRAY_BUFFER, vboPart.getFirst() * DefaultVertexFormats.BLOCK.getNextOffset(),
