...
public class OptifineTransformer {
	...
	public static void registerTransformers(IClassTransformerRegistry registry) {
		...
		registry.add("meldexun.nothirium.mc.renderer.chunk.RenderChunkTaskCompile", "renderBlockState", ClassWriter.COMPUTE_FRAMES, method -> {
			...
			method.instructions.insert(ASMUtil.first(method).varInsn("bufferBuilder", "Lnet/minecraft/client/renderer/WorldRenderer;").opcode(Opcodes.ASTORE).find(), ASMUtil.listOf(
					start,
					new VarInsnNode(Opcodes.ALOAD, ASMUtil.findLocalVariable(method, "bufferBuilder", "Lnet/minecraft/client/renderer/WorldRenderer;").index),
					new VarInsnNode(Opcodes.ALOAD, ASMUtil.findLocalVariable(method, "blockState", "Lnet/minecraft/block/state/IBlockState;").index),
					new VarInsnNode(Opcodes.ALOAD, ASMUtil.findLocalVariable(method, "pos", "Lnet/minecraft/util/BlockPos;").index),
					new MethodInsnNode(Opcodes.INVOKEVIRTUAL, "net/minecraft/client/renderer/WorldRenderer", "getRenderEnv", "(Lnet/minecraft/block/state/IBlockState;Lnet/minecraft/util/BlockPos;)Lnet/optifine/render/RenderEnv;", false),
		...
		registry.add("meldexun.nothirium.mc.renderer.chunk.SectionRenderCache", "calculateCombinedLight", ClassWriter.COMPUTE_FRAMES, method -> {
			method.instructions.insert(ASMUtil.last(method).opcode(Opcodes.IRETURN).findThenPrev().type(LabelNode.class).find(), ASMUtil.listWithLabel(label -> ASMUtil.listOf(
					...
					NothiriumClassTransformer.createObfMethodInsn(Opcodes.INVOKEVIRTUAL, "net/minecraft/block/Block", "func_149662_c", "()Z", false),
					new JumpInsnNode(Opcodes.IFNE, label),
					new VarInsnNode(Opcodes.ALOAD, ASMUtil.findLocalVariable(method, "pos", "Lnet/minecraft/util/BlockPos;").index),
					new VarInsnNode(Opcodes.ILOAD, ASMUtil.findLocalVariable(method, "light", "I").index),
					new MethodInsnNode(Opcodes.INVOKESTATIC, "net/optifine/DynamicLights", "getCombinedLight", "(Lnet/minecraft/util/BlockPos;I)I", false),
